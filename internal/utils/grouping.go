...
type Route struct {
	...
	Auth          string
	Tables        []string
	RequestModel    string // Name of the request model (from contracts.tf.rb)
	ResponseModel   string // Name of the response model (from contracts.tf.rb)
	ResponseContext string // Explicit context override for response model; empty = use Gateway
}

// ModelDefinition represents a JSON Schema model from contracts.tf.rb
